...
describe('runFileBFS', () => {
  ...
  it('stops propagation when weight drops below 0.1', () => {
    const seeds: SeedFile[] = [{ fileId: 'f1', reason: 'component_match' }]
    const edges: FileGraphEdge[] = [
      { from_file_id: 'f1', to_file_id: 'f2', edge_type: 'static' }, // depth 1: 0.7
      { from_file_id: 'f2', to_file_id: 'f3', edge_type: 'static' }, // depth 2: 0.49
      { from_file_id: 'f3', to_file_id: 'f4', edge_type: 'static' }, // depth 3: 0.343
      { from_file_id: 'f4', to_file_id: 'f5', edge_type: 'static' }, // depth 4: 0.24 (exceeds maxDepth=3, but use explicit maxDepth=10)
