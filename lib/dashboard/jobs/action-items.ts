...
export async function computeAndStoreActionItems(
  ...
  const { data: recentSnapshots, error: snapError } = await db
    ...
    .in('change_id', changeIds.length > 0 ? changeIds : ['__none__'])
  ...
  const { data: lowConf } = await db
    ...
    .in('component_id', compIds?.map(c => c.id) ?? ['__none__'])
    .eq('is_primary', true)
    .lt('confidence', 40)
  ...
  const { data: deps } = await db
    ...
    .select('to_id')
    .in('to_id', compIds?.map(c => c.id) ?? ['__none__'])
    .is('deleted_at', null)
  ...
  if (deleteError) { console.error('[action-items] delete failed:', deleteError) }
